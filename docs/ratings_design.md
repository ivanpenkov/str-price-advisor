...
flowchart TD
    ...
    subgraph NetworkLayer ["Proxy & Stealth Network Layer"]
        ...
        CHI["feeder-chi (NordVPN Chicago:1080)"]
...
sequenceDiagram
    ...
    CLI->>Proxies: Verify & acquire feeder ports (SF, DAL, CHI)
    CLI->>Collector: run_sync(platforms, force=False)
    par Concurrent Ingestion
        Collector->>Platforms: Airbnb PDP + GraphQL (feeder-sf)
        Collector->>Platforms: VRBO Apollo GraphQL + DOM (feeder-dal)
        Collector->>Platforms: Booking.com Review API + DOM (feeder-chi)
...
#### 1. Airbnb Ingestion
- **Proxy Feeder**: `feeder-sf-1` (`san-francisco.us.socks.nordhold.net:1080` -> HostRoyale, CA).
...
- **Proxy Feeder**: `feeder-chi-1` (`chicago.us.socks.nordhold.net:1080` -> HostRoyale / Datacamp, IL).
...
- **Network**: Direct HTTPS (fallback to `feeder-dal-1` if blocked).
