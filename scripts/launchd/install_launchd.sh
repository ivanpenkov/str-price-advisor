...
chmod +x "$SCRIPT_DIR/run_mobile_bridge.sh"
chmod +x "$PROJECT_ROOT/scripts/mobile_ntfy_bridge.py"
...
echo "  4. com.villasol.mobile-ntfy-bridge:  Continuous Two-Way Mobile Bridge (KeepAlive)"
echo ""
...
echo "  - Stream Mobile Bridge logs: tail -f $LOG_DIR/mobile_bridge.log"
echo "  - Run PMS Sync now:          bash $SCRIPT_DIR/run_pms_sync.sh"
echo "  - Run Quick Scan now:        bash $SCRIPT_DIR/run_daily_quickscan.sh"
echo "  - Manage Mobile Bridge:      bash $PROJECT_ROOT/scripts/install_mobile_bridge.sh --status"
